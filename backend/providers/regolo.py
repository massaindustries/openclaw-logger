...
class RegoloProvider(BaseLLMProvider):
        ...
        for attempt in range(max_retries):
            try:
                async with httpx.AsyncClient(timeout=120.0) as client:
                    ...
                    logger.debug("Regolo payload: %s", payload)
    ...
    def format_message(self, query: str, context: str | None) -> list[dict]:
        ...
        providers – a system message for instructions followed by a user
        message containing the logs (context) and the actual query.
        """
        user_content = query
        if context:
